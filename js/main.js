var config = {
    ...
    "cwLogin": { "LoginName": "<put your username here>", "Password": "<put your password here>" },
...
function submitToCityworks(submitTo) {
    ...
    $.post(config.cwApiUrl + "ServiceRequest/Create", { data: dojo.toJson(submit), token: token },
        function(data) {
            if (data.Status === 0) {
...
function problemSelected() {
    ...
    $.post(config.cwApiUrl + "ServiceRequest/QA", { data: dojo.toJson({ ProblemSid: sid }), token: token },
        function(data) {
            submitToFieldName = "";
...
function getProblems() {
    $.post(config.cwApiUrl + "ServiceRequest/Problems", { data: dojo.toJson({ ForPublicOnly: false }), token: token },
        function(data) {
...
function getRequestDetails(id) {
    $.post(config.cwApiUrl + "ServiceRequest/ById", { data: dojo.toJson({ RequestId: id }), token: token },
        function(data) {
...
function getRequestsByIds(ids) {
    $.post(config.cwApiUrl + "ServiceRequest/ByIds", { data: dojo.toJson({ RequestIds: ids }), token: token },
        function(data) {
            ...
            if (data.Value) {
                ...
                $(data.Value).each(function(i, req) {
                    ...
                    tbody.append("<tr><td>" + req.RequestId + "</td><td>" + req.Description + "</td><td>" + req.DateTimeInit.replace("T", " at ") + "</td></td></tr>");
                });
            }
...
function getOpenRequests(extent, sid) {
    ...
    $.post(config.cwApiUrl + "ServiceRequest/Search", { data: dojo.toJson(params), token: token },
        function(data) {
...
require(["esri/map", "esri/geometry/Point", "esri/SpatialReference", "esri/layers/ArcGISTiledMapServiceLayer", "dojo/domReady!"], function(Map, Point, SpatialReference, ArcGISTiledMapServiceLayer) {
    ...
    map = new Map("map", {
        center: new Point(2106217, 745583, new SpatialReference(2264)),
        zoom: 3,
        logo: false,
